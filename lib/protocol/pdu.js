...
SnmpStdPDU(arg)
{
	 ...
	 * getters and setters.
	 */
	if (this.op == SnmpPDU.GetBulkRequest) {
	...
	this.__defineSetter__(errst_if_name,
	    _set_bind(self, '_' + errst_if_name, 'number', 'Integer'));
	this.__defineGetter__(erridx_if_name, function () {
		return (self._error_index.value);
	});
	f = _set_bind(self, '_' + erridx_if_name, 'number', 'Integer');
	if (this.op == SnmpPDU.GetBulkRequest) {
...
SnmpStdPDU.prototype.encode = function (writer) {
	//console.log("SnmpStdPDU encode");
	var i;

	if (this._op == SnmpPDU.GetBulkRequest &&
	...
	//console.log("_request_id writer = " , writer);
	this._request_id.encode(writer);
	//console.log("_error_status writer = " , writer);
	this._error_status.encode(writer);
	//console.log("_error_index writer = " , writer);
	this._error_index.encode(writer);
	//console.log("end writer = " , writer);
