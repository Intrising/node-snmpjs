...
{
	//console.log("SnmpMessage encode=",this);
	...
	if(this.version != 3){
		...
		while(time_value > 0xff){
			time_value = time_value>>8;
			time_length++;
		}
		console.log("time_length = " , time_length );
		len = 2 + 12;
		len += this._v3.secmsg.EngineID._value.length + /*EngineBoots*/ 1 + /*EngineTime*/time_length + this._v3.secmsg.userName._value.length + this._v3.secmsg.params._value.length + this._v3.secmsg.privacyParams._value.length;
