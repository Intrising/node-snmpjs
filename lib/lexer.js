...
Lexer.prototype.simlex = function ()
{
	var tag = this.reader.peek();
	var soff = this.reader.offset + 1;
	var lenlen = this.reader.readLength(soff) - soff;
  ...
  tokobj= { tag: tag, token: token, len: 0, buf:null}
	if (token_len > 0) {
    tokobj.len = token_len-2;
		tokobj.buf = new Buffer( this.reader.buffer.slice(2, token_len));
	}
