...
Agent.prototype._transmit_response = function _transmit_response(rsp) {
	...
	rsp.encode();
	//console.log("Agent._transmit_response , rsp.encode=",rsp);
...
Agent.prototype._process_req = function _process_req(req) {
	...
	if (req.version >= 0 && req.version <= 1) {
		...
		console.log("v3 " , req.pdu.v3);
		//console.log("op= " , req.pdu.op);
		//console.log("pdu= " , req.pdu.v3);
		//console.log("varbinds= " , req.pdu.varbinds);
		//console.log("req.pdu.v3.secmsg.EngineID=",req.pdu.v3.secmsg.EngineID);
		req.pdu.v3.header.flags._value = new Buffer(1).clear();
		id = [0x80,0x00,0x14,0x55,0x03,0x00,0x40,0xc7,0x1c,0x70,0x22];
		req.pdu.v3.secmsg.EngineID._value = new Buffer(id);
		req.pdu.v3.context.engineID._value = new Buffer(id);
		rsp = message.createMessage({ version: req.version , v3:req.pdu.v3});
		rsp.dst = req.src;
		
		rsp.pdu = PDU.createPDU({ op: PDU.Report,
		    request_id: req.pdu.request_id });
		rsp.pdu.error_status = 0;
		rsp.pdu.error_index = 0;
		//console.log("rsp=",rsp);
