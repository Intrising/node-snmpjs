 ...
 * response object.
 */
Agent.prototype._varbind_set_single = function _varbind_set_single(req, rsp, idx, vb) {
  //console.log("_varbind_set_single : vb = " , vb);
...
Agent.prototype._transmit_response = function jh(rsp,error) {
  ...
  this._conn.send(rsp.raw.buf, 0, rsp.raw.len, dst.port, dst.address, function(err, bytes) {
    //sock.close();
  });
};
...
Agent.prototype._getnext_lookup = function _getnext_lookup(oid, exactok) {
  ...
  oid = addr.join('.');
  node = this._mib.next_lookup(oid);
  //console.log("_getnext_lookup node.oid = " , node.oid);
  if (!node)
    return (null);
...
Agent.prototype._process_req = function _process_req(req,action) {
  //console.log("Agent._process_req , req varbinds = " ,req.pdu.varbinds);
  
  var rsp;
  if (typeof req.version === "undefined" && req.version === null){
    console.log("version is undefined , return!!");
    return;
  }
  if (req.version >= 0 && req.version <= 1) {
    ...
    //set snmp
    if(this._flag_write === 'writable'){
      var xx, yy, _i, _j, _len, _len1, _ref, _ref1;

      var write_count = 0;
      _ref = this._write_request;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        xx = _ref[_i];
        _ref1 = this._set_items;
        for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
          yy = _ref1[_j];
          if (xx.oid === yy.oid) {
            //console.log("yy.data.value=",yy.data.value);
            xx.handler(yy.data.value);
          }
        }
      }      
    }
    ...
    console.log("error_action = " , error_action , " this._flag_write = " , this._flag_write);
    //set snmp
    if((error_action === 'none' || error_action ==='authentication_success') && this._flag_write === 'writable'){
      var xx, yy, _i, _j, _len, _len1, _ref, _ref1;
    ...
    if((error_action === 'none' || error_action ==='authentication_success') || error_action === 'wait_decrypted'){
    ...
    if (flag & 0x01) {
      ...
      if ((auth_data.key != null) && (auth_data.auth_params != null)) {
        ...
        if(params.slice(0,12).toString("hex") !== auth_data.auth_params.toString("hex")){
          ...
          security.error_action = 'authentication_success';
        }
      }
    }
...
Agent.prototype._rebuild_varbinds = function _rebuild_varbinds(items) {
  //console.log("_rebuild_varbinds:items= ",items);
  var buf_array, item, null_item, offset, offset_pdu, packet_total_length, pdu, x, y, _i, _j, _len, _len1;
  buf_array = [];
  packet_total_length = 0;
  null_item = new Buffer([0x05, 0x00]);
  for (_i = 0, _len = items.length; _i < _len; _i++) {
    x = items[_i];
    offset = 0;
    item = new Buffer(2 + 2 + x.oid.length + 2);
    item[offset] = 0x30;
    offset++;
    ...
    item[offset] = 0x06;
    offset++;
    item[offset] = x.oid.length;
    offset++;
    offset += x.oid.copy(item, offset);
    offset += null_item.copy(item, offset);
    packet_total_length += offset;
    buf_array.push(item);
  }
  offset_pdu = 0;
  pdu = new Buffer(packet_total_length + 2);
  pdu[offset_pdu] = 0x30;
  offset_pdu++;
  pdu[offset_pdu] = packet_total_length;
  offset_pdu++;
  for (_j = 0, _len1 = buf_array.length; _j < _len1; _j++) {
    y = buf_array[_j];
    offset_pdu += y.copy(pdu, offset_pdu);
  }
  return pdu;
};

Agent.prototype._hack_set_op = function _hack_set_op(pdu,req) {
    //console.log("_hack_set_op = " , req.pdu.varbinds);
    var community_data, community_length, community_start, data_length_offset, data_op, error_index_data, error_index_length, error_index_start, error_status_data, error_status_length, error_status_start, item, item_data, item_length, item_start, item_total_length, items, keep_buf, offset, offset_data, offset_rebuild_pdu, rebuild_pdu, request_id_data, request_id_length, request_id_start, varbinds, version_data, version_length, version_start;
    this._set_items = [];
    this._hack_info = {};
    this._hack_info.info_pos = {};
    offset = 0;
    version_start = this._find_version_pos(pdu, new Buffer([0x02, 0x01]));
    version_length = pdu[version_start + 1];
    version_data = pdu.slice(version_start + 2, (version_start + 1 + version_length) + 1 || 9e9);
    offset = version_start + 2 + version_length;
    if (version_data[0] === 0 || version_data[0] === 1) {
      community_start = offset;
      community_length = pdu[community_start + 1];
      community_data = pdu.slice(community_start + 2, (community_start + 1 + community_length) + 1 || 9e9);
      offset = community_start + 2 + community_length;
    }
    else if (version_data[0] === 3) {
      msg_global_data_start =  offset;
      msg_global_data_length = pdu[msg_global_data_start + 1];
      msg_global_data = pdu.slice(msg_global_data_start + 2, (msg_global_data_start + 1 + msg_global_data_length) + 1 || 9e9);
      offset = msg_global_data_start + 2 + msg_global_data_length;

      msg_other_start =  offset;
      msg_other_length = pdu[msg_other_start + 1];
      msg_other_data = pdu.slice(msg_other_start + 2, (msg_other_start + 1 + msg_other_length) + 1 || 9e9);
      offset = msg_other_start + 2 + msg_other_length;

      msg_data_start =  offset;
      msg_other_length = pdu[msg_data_start + 1];
      msg_other_data = pdu.slice(msg_other_start + 2, (msg_other_start + 1 + msg_other_length) + 1 || 9e9);
      //offset = msg_other_start + 2 + msg_other_length;
      
      context_engine_id_start = offset + 2;
      context_engine_id_length = pdu[context_engine_id_start + 1];
      context_engine_id_data = pdu.slice(context_engine_id_start + 2, (context_engine_id_start + 1 + context_engine_id_length) + 1 || 9e9);
      offset = context_engine_id_start + 2 + context_engine_id_length;
      
      context_name_start = offset;
      context_name_length = pdu[context_name_start + 1];
      context_name_data = pdu.slice(context_name_start + 2, (context_name_start + 1 + context_name_length) + 1 || 9e9);
      offset = context_name_start + 2 + context_name_length;
    }
    data_op = pdu[offset] & 0x0f;
    data_length_offset = offset + 1;
    if (data_op === 3) {
      this._flag_write = 'writable';
      
      this._hack_info.info_pos.op = offset;
      
      pdu[offset] = 0xa0;
      request_id_start = offset + 2;
      request_id_length = pdu[request_id_start + 1];
      request_id_data = pdu.slice(request_id_start + 2, (request_id_start + 1 + request_id_length) + 1 || 9e9);
      offset = request_id_start + 2 + request_id_length;
      error_status_start = offset;
      error_status_length = pdu[error_status_start + 1];
      error_status_data = pdu.slice(error_status_start + 2, (error_status_start + 1 + error_status_length) + 1 || 9e9);
      offset = error_status_start + 2 + error_status_length;
      
      this._hack_info.info_pos.error_status = error_status_start+2;
      
      error_index_start = offset;
      error_index_length = pdu[error_index_start + 1];
      error_index_data = pdu.slice(error_index_start + 2, (error_index_start + 1 + error_index_length) + 1 || 9e9);
      offset = error_index_start + 2 + error_index_length;
      
      this._hack_info.info_pos.error_index = error_index_start+2;
      
      keep_buf = pdu.slice(0, (offset - 1) + 1 || 9e9);
      item_total_length = pdu[offset + 1];
      offset = offset + 2;
      var items_array = [];
      while (item_total_length > 0) {
        item_start = offset;
        item_length = pdu[item_start + 1];
        item_data = pdu.slice(item_start + 2, (item_start + 1 + item_length) + 1 || 9e9);
        item = {};
        offset_data = 0;
        item.oid = item_data.slice(offset_data + 2, (offset_data + 2 + item_data[1] - 1) + 1 || 9e9);
        offset_data = offset_data + 2 + item_data[1];
        item.value = item_data.slice(offset_data + 2, (offset_data + 1 + item_data[offset_data + 1]) + 1 || 9e9);
        items_array.push(item);
        offset = item_start + 2 + item_length;
        item_total_length = item_total_length - 2 - item_length;
      }
      
      if (version_data[0] === 0 || version_data[0] === 1) {
        var community, x, y, _i, _len, _ref;
        ...
        community = (function() {
          ...
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            x = _ref[_i];
            _results.push(x.community);
          }
          return _results;
        }).call(this);
      
        if (community.indexOf(community_data.toString()) >= 0) {
          _ref = this._community;
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            y = _ref[_i];
            if (community_data.toString() === y.community) {
              if (y.readonly) {
                this._flag_write = 'not_writable';
                this._hack_info.buf = pdu;
                return this._hack_info;
              }
            }
          }
        } else {
      ...
      //console.log("req.pdu.varbinds=",req.pdu.varbinds);
      _ref2 = req.pdu.varbinds;
      //console.log("oids=",oids);
      for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
        y = _ref2[_i];
        //console.log("y=",y);
        if (oids.indexOf(y.oid) < 0) {
          this._flag_write = 'no_creation';
          this._hack_info.buf = pdu;
          return this._hack_info;
        }
      }
      
      //process varbinds to set_items
      _ref2 = req.pdu.varbinds;
      //console.log("oids=",oids);
      for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
        z = _ref2[_i];
        ...
        tmp.oid = z.oid;
        tmp.data = {};
        tmp.data.value = z.data.value;
        this._set_items.push(tmp);
      }
      
      varbinds = this._rebuild_varbinds(items_array);
      rebuild_pdu = new Buffer(keep_buf.length + varbinds.length);
      offset_rebuild_pdu = 0;
      offset_rebuild_pdu += keep_buf.copy(rebuild_pdu, offset_rebuild_pdu);
      offset_rebuild_pdu += varbinds.copy(rebuild_pdu, offset_rebuild_pdu);
      if(rebuild_pdu[1] === 0x81){
        rebuild_pdu[2] +=  rebuild_pdu.length - 2 + 1;
      }
      else if(rebuild_pdu[1] === 0x82){
        rebuild_pduf[3] +=   rebuild_pdu.length - 2 + 2;
      }
      else{
        rebuild_pdu[1] = rebuild_pdu.length - 2;
      }
      
      rebuild_pdu[data_length_offset] = rebuild_pdu[1] - data_length_offset + 1;
      this._hack_info.buf = rebuild_pdu;
...
Agent.prototype._recv = function _recv(raw, src) {
  ...
  var action = 'none';
  this._flag_write = 'no_error';
  
  try {
    this._process_action = this._is_v3_process(raw.buf);
    //console.log("this._process_action.error_action=",this._process_action.error_action);
    if (this._process_action.error_action === 'wait_decrypted') {
      ...
      pre_req = message.parseMessage({ raw: {buf:this._process_action.decrypted_buf}, src: src });
      hack_buf = this._hack_set_op(this._process_action.decrypted_buf,pre_req);
      //console.log("this._flag_write=",this._flag_write);
      if (this._flag_write === 'writable' || this._flag_write === 'no_error') {
        req = message.parseMessage({ raw: {buf:hack_buf.buf}, src: src });
        //console.log("req=",req);
      }
      else{
        //console.log("hack_buf.info_pos=",hack_buf.info_pos);
        hack_buf.buf[hack_buf.info_pos.op] = 0xa2;
        hack_buf.buf[hack_buf.info_pos.error_index] = this._set_items.length;
        if (this._flag_write === 'no_creation'){
          hack_buf.buf[hack_buf.info_pos.error_status] = 0x0b;
        }
        else if (this._flag_write === 'not_writable'){
          hack_buf.buf[hack_buf.info_pos.error_status] = 0x11;
        }
        
        //console.log("hack_buf.buf = ",hack_buf.buf);
        //sock = dgram.createSocket(src.family);
        this._conn.send(hack_buf.buf, 0, hack_buf.buf.length, src.port, src.address, function(err, bytes) {
          //sock.close();
        });
      }
    }
      ...
      pre_req = message.parseMessage({ raw: raw, src: src });
      hack_buf = this._hack_set_op(raw.buf,pre_req);
      //console.log("this._flag_write=",this._flag_write);
      if (this._flag_write === 'writable' || this._flag_write === 'no_error') {
        req = message.parseMessage({ raw: {buf:hack_buf.buf}, src: src });
      }
...
Agent.prototype.get_conn = function get_conn() {
  return this._conn;
}
...
Agent.prototype.bind = function bind(arg) {
  ...
  this._conn = dgram.createSocket(arg.family);
  this._conn.on('message', function _recv_binder(msg, rinfo) {
  ...
  this._conn.bind(arg.port, arg.addr);
  this._log.info('Bound to ' + this._conn.address().address + ':' +
      this._conn.address().port);
};

Agent.prototype.request_write = function request_write(prov) {
  //console.log("Agent.request , oid = " ,oid,"handler=", handler, "columns=",columns);
  //prov[0].handler("test", (function(result) {
    //return console.log("test , result = ", result);
  //}));
  this._write_request = prov;
  //console.log("this._write_request=",this._write_request);
};
