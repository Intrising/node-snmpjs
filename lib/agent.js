...
Agent.prototype._transmit_response = function jh(rsp,error) {
  ...
  sock.send(rsp.raw.buf, 0, rsp.raw.len, dst.port, dst.address, function(err, bytes) {
    sock.close();
  });
};
  ...
  try {
    ...
    if (this._process_action.error_action === 'wait_decrypted') {
      ...
      if (this._flag_write === 'writable' || this._flag_write === 'no_error') {
        ...
        sock.send(hack_buf.buf, 0, hack_buf.buf.length, src.port, src.address, function(err, bytes) {
          sock.close();
        });
      }
    }
