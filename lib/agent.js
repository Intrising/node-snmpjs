...
Agent.prototype._check_error = function _check_error(req) {
	...
	if (typeof req.pdu.v3.secmsg.userName._value[0] !== "undefined" && req.pdu.v3.secmsg.userName._value[0] !== null){
		if(!this._user_match(req.pdu.v3.secmsg.userName._value.toString(),'auth_nopriv')){
	...
	if (typeof req.pdu.v3.secmsg.params._value[0] !== "undefined" && req.pdu.v3.secmsg.params._value[0] !== null){
		if(req.pdu.v3.header.flags._value[0] & 1){
			...
			buf.fill(0x00,pos,pos+12);
			if (typeof key !== "undefined" && key !== null) {
				var params = crypto.createHmac('md5', key).update(buf).digest("hex");
				if(params !== req.pdu.v3.secmsg.params._value.toString()){
...
Agent.prototype._process_req = function _process_req(req,action) {
	...
	if (req.version >= 0 && req.version <= 1) {
		...
		var error_action;
		if(action === ' none'){
			error_action = this._check_error(req);
		}
		else{
			error_action = action;
		}
...
Agent.prototype._is_v3_encrypted = function _is_v3_encrypted(snmp_pdu){ 
...
Agent.prototype._recv = function _recv(raw, src) {
	...
	var action = 'none';
	try {
		var encrypted_buf = this._is_v3_encrypted(raw.buf);
		if (encrypted_buf[0] != null) {
			is_encrypted = true;
			req = message.parseMessage({ raw: {buf:encrypted_buf}, src: src });
		...
		console.log("XXX in some cases we can reply with an error , err = ", err , "is_encrypted = ",is_encrypted); 
