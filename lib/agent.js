...
Agent.prototype._process_req = function _process_req(req) {
	...
	if (req.version >= 0 && req.version <= 1) {
		...
		req.pdu.v3.secmsg.EngineBoots._value = 1;
...
Agent.prototype.access_users = function(action, name, security_level, authentication_protocol, authentication_password, privacy_protocol, privacy_password,snmp_engine_id) {
	var self, user_match, x , AuthMD5_password_to_key;
	...
	if (action === 'join' && !user_match(name)) {
		tmp = {
			name: name,
	...
	} else if (action === 'leave' && user_match(name)) {
